document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      if (field.hasAttribute('min') && (type === 'number' || type === 'range')) {
        ...
        if (parseFloat(value) < min) {
          showFieldError(field, getErrorMessage('min', name, min));
          isValid = false;
          return;
        }
      }
      ...
      if (field.hasAttribute('max') && (type === 'number' || type === 'range')) {
        ...
        if (parseFloat(value) > max) {
          showFieldError(field, getErrorMessage('max', name, max));
          isValid = false;
          return;
        }
      }
      ...
      if (field.hasAttribute('pattern')) {
        ...
        if (!pattern.test(value)) {
          const title = field.getAttribute('title') || getErrorMessage('pattern', name);
    ...
    if (!isValid) {
      const firstError = form.querySelector('.field-error-message');
      if (firstError) {
        firstError.closest('.field-wrapper, .form-group, div') &&
  ...
  function getErrorMessage(type, fieldName, param) {
    ...
    const messages = {
      ...
      max: label + ' must be no more than ' + param + '.',
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '397c03da-4b27-4bfb-a749-d53be9a94f93',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '397c03da-4b27-4bfb-a749-d53be9a94f93';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '397c03da-4b27-4bfb-a749-d53be9a94f93';
